...
if __name__ == '__main__':
    ...
    search_output = args.keywords if args.keywords!= None else input("[*] Enter Linkedin Search Keyword (eg. \"google\"): ")
    ...
    if bCompany:
        while True:
            bSpecific = args.companyid if args.companyid!= None else input("[*] Provide \"facetCurrentCompany=\" ID: ")
            if bSpecific != 0:
                try:
                    int(bSpecific)
                    companyID = bSpecific
    ...
    # Email 
    while True:
        suffix = args.email if args.email!= None else input("[*] Enter Email Domain (eg. gmail.com): ")
        suffix = suffix.lower()
        if "." in suffix:
            break
        else:
    ...
    # Username Scheme
    while True:
        prefix = args.naming if args.naming!= None else input("[*] Naming Scheme for the Company \n    [0] Auto (hunter.io) \n    [1] FirstLast \n    [2] FirstMiddleLast \n    [3] FLast \n    [4] FirstL \n    [5] First.Last \n    [6] Last.First \n    [!] Select: ")
        prefix = prefix.lower()
